...
```{r average_price_upc_max_after_week130, echo=TRUE, eval =FALSE}
data %>%  filter(store == 122, upc == 197, week >130) %>% summarize(average_price = mean(price))
```
```{r average_price_upc_max_before_week130, echo=TRUE,eval=FALSE}
data %>%  filter(store == 122, upc == 197, week < 130) %>% summarize(average_price = mean(price))
```
